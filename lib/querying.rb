def select_books_titles_and_years_in_first_series_order_by_year
  "SELECT books.title, books.year FROM series INNER JOIN books ON series.id = books.series_id WHERE books.series_id = 1"
end
...
def select_name_and_series_subgenres_of_authors
  "SELECT authors.name, subgenres.name FROM ((series INNER JOIN authors ON authors.id = series.author_id)
